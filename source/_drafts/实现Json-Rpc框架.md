...
- 在多线程网络通信中，存在响应时序问题，无法保证响应与请求一一对应，这是危险的，Requestor模块则提供了解决这一问题的方案。
	- 例如：异步IO网络通信库（如Muduo）不提供同步接收接口，而是在数据读取后调用回调处理。

- 解决方案：创建请求管理模块，为每个请求分配请求ID。服务端响应时包含请求ID。客户端将响应存储在以请求ID为键的hash_map中，并提供阻塞接口获取指定请求ID的响应，确保请求与响应正确匹配。
